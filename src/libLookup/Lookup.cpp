...
void Lookup::SendMessageToLookupNodes(const bytes& message) const {
  ...
  if (QUERY_DNS_FOR_SEED) {
    querySuccess = ObtainIpListFromDns(ipList, MULTIPLIER_DNS);
    if (querySuccess) {
...
void Lookup::SendMessageToSeedNodes(const bytes& message) const {
  ...
  bool querySuccess = false;
  if (QUERY_DNS_FOR_SEED) {
    querySuccess = ObtainIpListFromDns(seedIps, UPPER_SEED_DNS);
  }

  // If failed to obtain from dns list, use local list
  if (!querySuccess) {
...
void Lookup::SendMessageToRandomL2lDataProvider(const bytes& message) {
  ...
  bool querySuccess = false;
  if (QUERY_DNS_FOR_SEED)
    querySuccess = ObtainIpListFromDns(ipFromDns, L2L_DATA_PROVIDERS_DNS);
...
void Lookup::RemoveSeedNodesFromBlackList() {
  ...
  bool querySuccess = false;
  if (QUERY_DNS_FOR_SEED)
    querySuccess = ObtainIpListFromDns(ipList, UPPER_SEED_DNS);
